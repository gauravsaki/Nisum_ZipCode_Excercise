...
public class ZipCodeParserTest {
	...
	public void testIllegalValues() {
		List<ZipCodeRange> ranges = ZipCodeParser.parse(new String[] {"a_reasonable_mistake", "[1,2,3]", "[30000,40000]"});
		
		assertEquals("Two ranges were parsed", ranges.size(), 1);
		
		assertEquals("Low end parsed on second range", ranges.get(0).low(), 30000);
		assertEquals("High end parsed on second range", ranges.get(0).high(), 40000);
	}
